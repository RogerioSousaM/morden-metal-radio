...
const TopMonthPopup = () => {
  ...
  const [topMonthData, setTopMonthData] = useState<TopMonthData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // Carregar dados do Top do Mês
    const loadTopMonthData = async () => {
      try {
        const response = await fetch('/api/top-month')
        if (response.ok) {
          const data = await response.json()
          setTopMonthData(data)
        }
      } catch (error) {
        console.error('Erro ao carregar dados do Top do Mês:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (bandasSection && !isDismissed && topMonthData?.isActive) {
  ...
  // Não mostrar se não há dados ou se não está ativo
  if (loading || !topMonthData?.isActive) {
    return null
  }

  const data = topMonthData || fallbackData
  const initials = data.bandName?.split(' ').map(n => n[0]).join('').slice(0, 2) || fallbackData.initials
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-gradient-to-br from-metal-orange/20 to-metal-accent/20 rounded-lg flex items-center justify-center">
                  <span className="text-metal-orange font-bold text-sm">{initials}</span>
                </div>
                <div>
                  <h4 className="text-metal-text font-bold">{data.bandName}</h4>
                  <p className="text-metal-text-secondary text-sm">{data.albumName}</p>
                </div>
              </div>
              
              <p className="text-metal-text-secondary text-sm">
                A banda mais tocada do mês com mais de {formatPlayCount(data.playCount)} reproduções
              </p>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => window.open(data.newsLink, '_blank')}
              >
